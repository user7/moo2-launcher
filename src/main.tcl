...
"
    ...
    set enable_cfg "# DO NOT EDIT!
#
# This file lists enabled mods. It is generated automatically by MoO2 Launcher
# every time it starts the game, so all manual edits are lost.
...
proc enum_widgets {fname flabel args} {
    ...
    foreach a $args {
        ...
        set cwsuffix .c$r
        if {[dict exists $d cwsuffix]} {
            set cwsuffix [dict get $d cwsuffix]
        }
        dict set d cwidget $fname$cwsuffix
        dict set d lwidget ${fname}.l$r
        dict set d var ::${fname}.$r
        lappend wlist $d
        incr r
    }
...
proc create_dosbox_frame {frame_info} {
    ...
    foreach w [dict get $frame_info wlist] {
        ...
        if {[dict exists $w options]} {
            ...
            if {[dict exists $w update]} {
                bind $wi <<ComboboxSelected>> [dict get $w update]
            }
        } elseif {[dict exists $w checkbox]} {
            # checkbox
            ttk::checkbutton $wi -variable $wvar
